    ...
    with tqdm(total=len(products), desc="Downloading main images") as pbar:
        for i in range(len(products)):
            try:
                ...
                downloaded_path = download_image(products[i].main_photo_link, file_path, overwrite=overwrite)
                if downloaded_path:
                    # Use the sanitized path returned by download_image
                    # ZDE - Přiřazení sanitizované cesty (bez části za znakem procenta) do main_photo_filepath
                    products[i].main_photo_filepath = os.path.abspath(downloaded_path)
                pbar.update(1)
        ...
        for i in range(len(products)):
            try:
                ...
                for link in products[i].photogallery_links:
                    ...
                    downloaded_path = download_image(link, file_path, overwrite=overwrite)
                    if downloaded_path:
                        # Use the sanitized path returned by download_image
                        # ZDE - Přidání sanitizované cesty (bez části za znakem procenta) do photogallery_filepaths
                        products[i].photogallery_filepaths.append(os.path.abspath(downloaded_path))
                    pbar.update(1)
            except Exception as e:
