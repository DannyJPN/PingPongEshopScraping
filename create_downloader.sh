...
mkdir -p ${BRAND_LOWER}downloader/${BRAND_LOWER}lib
cp -r butterflydownloader/shared ${BRAND_LOWER}downloader/

# Create constants.py
cat > ${BRAND_LOWER}downloader/${BRAND_LOWER}lib/constants.py << EOF
"""
Constants specific to ${BRAND_DISPLAY} e-shop.
"""
...
DEFAULT_RESULT_FOLDER = r"H:/Desaka/${BRAND_PASCAL}"

# Main page filename
MAIN_PAGE_FILENAME = "${BRAND_LOWER}.html"

# CSV output filename
CSV_OUTPUT_NAME = "${BRAND_PASCAL}Output.csv"

# Log directory
LOG_DIR = r"H:/Logs"
EOF

# Copy and adapt other modules
for file in category_link_extractor category_pages_link_extractor product_detail_link_extractor product_attribute_extractor; do
  cp butterflydownloader/butterflylib/${file}.py ${BRAND_LOWER}downloader/${BRAND_LOWER}lib/
  sed -i "s/butterflylib/${BRAND_LOWER}lib/g" ${BRAND_LOWER}downloader/${BRAND_LOWER}lib/${file}.py
  sed -i "s/Butterfly/${BRAND_DISPLAY}/g" ${BRAND_LOWER}downloader/${BRAND_LOWER}lib/${file}.py
done

# Create main downloader script
cat > ${BRAND_LOWER}downloader/${BRAND_LOWER}_downloader.py << 'EOF2'
"""
${BRAND_DISPLAY} E-shop Downloader

Downloads product data from ${BRAND_DISPLAY} e-shop and exports to CSV.

Usage:
    python ${BRAND_LOWER}_downloader.py --result_folder H:/Desaka/${BRAND_PASCAL} --debug --overwrite
"""
...
from ${BRAND_LOWER}lib.constants import DEFAULT_RESULT_FOLDER, MAIN_URL, MAIN_PAGE_FILENAME, CSV_OUTPUT_NAME, LOG_DIR
from ${BRAND_LOWER}lib.category_link_extractor import extract_category_links
from ${BRAND_LOWER}lib.category_pages_link_extractor import extract_all_category_pages_links
from ${BRAND_LOWER}lib.product_detail_link_extractor import extract_all_product_detail_links
from ${BRAND_LOWER}lib.product_attribute_extractor import extract_products


def parse_arguments():
    """Parse command line arguments."""
    parser = ArgumentParser(description="${BRAND_DISPLAY} E-shop Downloader")
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("${BRAND_DISPLAY} Downloader Started")
    logging.info("=" * 80)
...
sed -i "s/\${BRAND_LOWER}/${BRAND_LOWER}/g" ${BRAND_LOWER}downloader/${BRAND_LOWER}_downloader.py
sed -i "s/\${BRAND_PASCAL}/${BRAND_PASCAL}/g" ${BRAND_LOWER}downloader/${BRAND_LOWER}_downloader.py
sed -i "s/\${BRAND_DISPLAY}/${BRAND_DISPLAY}/g" ${BRAND_LOWER}downloader/${BRAND_LOWER}_downloader.py

echo "Created ${BRAND_LOWER}downloader successfully!"
