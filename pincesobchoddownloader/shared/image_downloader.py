...
def download_image(url, filepath, overwrite=False, debug=False):
    try:
        logging.debug(f"Starting download_image function for URL: {url} with overwrite={overwrite}")
        ...
        logging.debug(f"Downloaded image to {sanitized_filepath}")  # Change from logging.info to logging.debug
