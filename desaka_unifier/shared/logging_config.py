...
def setup_logging(debug=False, log_file="logs/logfile.log"):
    ...
    # Set up the file handler with the formatter and UTF-8 encoding
    ...
    # Configure OpenAI library logging to show HTTP requests at DEBUG level only
    openai_logger = logging.getLogger('openai')
    openai_logger.setLevel(logging.WARNING if not debug else logging.DEBUG)

    # Configure httpx logger (used by OpenAI) to show HTTP requests at DEBUG level only
    httpx_logger = logging.getLogger('httpx')
    httpx_logger.setLevel(logging.WARNING if not debug else logging.DEBUG)
